...
export type OrderType = {
  ...
  _rev: string;
  nameNumber?: string;
  stripeCheckoutSeccionId?: string;
...
export type AllSanitySchemaTypes = SanityImagePaletteSwatch | SanityImagePalette | SanityImageDimensions | SanityFileAsset | Geopoint | SalesType | OrderType | ProductType | Category | Slug | BlockContent | SanityImageCrop | SanityImageHotspot | SanityImageAsset | SanityAssetSourceData | SanityImageMetadata;
...
// Query: *[            _type == "productType" && references(*[_type == "category" && slug.current == $category]._id)        ] | order(name asc)
...
// Source: ./sanity/lib/sale/gerActiveSaleByCuponCode.tsx
// Variable: ACTIVE_SALE_BY_COUPON_QUERY
// Query: *[_type == "salesType"        && couponCode == $couponCode        && isActive == true        && validFrom <= now()        && validUntil >= now()]        | order(validFrom desc)[0]
